...
export default function BcbpViewer({ parsed }: BcbpViewerProps) {
  // Define colors for segments to cycle through
  const colors = [
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <DetailCard label="Passenger Name" value={parsed.data.passengerName} icon="👤" />
          <DetailCard label="PNR / Booking Ref" value={parsed.data.pnr} icon="🔖" />
          <DetailCard label="Flight" value={`${parsed.data.carrier}${parsed.data.flightNumber}`} icon="✈️" />
          <DetailCard label="Route" value={`${parsed.data.fromCity} ➝ ${parsed.data.toCity}`} icon="🌍" />
          <DetailCard label="Seat" value={parsed.data.seat} icon="💺" />
          <DetailCard label="Date (Julian)" value={parsed.data.julianDate} icon="📅" />
          <DetailCard label="Sequence" value={parsed.data.checkInSeq} icon="🔢" />
          <DetailCard label="Status" value={parsed.data.passengerStatus} icon="ℹ️" />
        </div>
      </div>
